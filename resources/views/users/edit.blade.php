<x-app-layout>
                ...
                <form action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full px-4 py-2 rounded-xl border border-slate-200 focus:ring-2 focus:ring-cyan-200" required>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Email</label>
                            <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full px-4 py-2 rounded-xl border border-slate-200 focus:ring-2 focus:ring-cyan-200" required>
                        </div>
                    </div>
                    ...
                    @else
                        <div id="faculty_section" style="display: none;">
                            ...
                            <select name="faculty" class="w-full px-4 py-2 rounded-xl border border-slate-200 focus:ring-2 focus:ring-cyan-200">
                                <option value="" disabled>Pilih Fakultas...</option>
                                @foreach(\App\Models\Building::distinct()->pluck('fakultas') as $fakultas)
                                    @if($fakultas) 
                    ...
                    <div id="cctv_access_section" class="bg-slate-50 p-4 rounded-xl border border-slate-200" style="display: none;">
                        <h3 class="text-sm font-bold text-slate-700 mb-3">Pilih Hak Akses CCTV</h3>
                        <p class="text-xs text-slate-500 mb-4">Centang kamera yang boleh dipantau oleh user ini.</p>
    ...
    <script>
        function toggleSections() {
            ...
            // 1. CCTV Section (User Biasa)
            if (cctvSection) {
                cctvSection.style.display = (role === 'user') ? 'block' : 'none';
            }

            // 2. Faculty Section (Hanya jika elemen ada / Admin yg buka)
            if (facultySection) {
