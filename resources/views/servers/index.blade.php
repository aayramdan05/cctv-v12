<x-app-layout>
    <main id="main-content" class="pt-20 p-6 md:p-8" x-data="{ deployModalOpen: false }">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <div class="flex items-center gap-3">
                <!-- Gunakan @click.stop untuk mencegah bubbling dan pastikan Alpine ter-load -->
                <button @click.prevent="deployModalOpen = true; document.body.style.overflow='hidden'" class="px-4 py-2.5 rounded-xl bg-white text-slate-600 font-medium border border-slate-200 hover:bg-slate-50 hover:text-cyan-600 transition-all flex items-center gap-2 shadow-sm z-10 cursor-pointer" style="pointer-events: auto !important;">
        ...
        <div x-show="deployModalOpen" 
            ...
            <div @click.away="deployModalOpen = false; document.body.style.overflow='auto'" class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden relative">
                ...
                <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-gradient-to-r from-slate-50 to-white">
                    ...
                    <button @click="deployModalOpen = false; document.body.style.overflow='auto'" class="w-8 h-8 flex items-center justify-center rounded-full text-slate-400 hover:bg-red-50 hover:text-red-500 transition">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                            ...
                            <div class="p-5 text-sm text-slate-600 space-y-4">
                                <p>Login via SSH, lalu jalankan perintah berikut:</p>
                                <div class="space-y-4">
                                    <div class="border border-slate-100 rounded-lg p-3 bg-slate-50">
                                        <p class="mb-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2"><i class="fab fa-github text-slate-700 text-sm"></i> A. Clone Repository (Sparse-Checkout)</p>
                                        <pre class="bg-slate-900 border border-slate-700 text-cyan-300 p-3 rounded-lg text-xs overflow-x-auto font-mono select-all"><code>mkdir -p ~/cctv-setup && cd ~/cctv-setup
git init
...
git pull origin main</code></pre>
                                    ...
                                    <div class="border border-slate-100 rounded-lg p-3 bg-slate-50">
                                        ...
                                        <pre class="bg-slate-900 border border-slate-700 text-cyan-300 p-3 rounded-lg text-xs overflow-x-auto font-mono select-all"><code>cd scripts
...
location /node2/ {
    ...
    proxy_set_header Origin "";
                ...
                <div class="p-5 border-t border-slate-100 bg-white flex justify-end">
                    <button @click="deployModalOpen = false; document.body.style.overflow='auto'" class="px-6 py-2.5 rounded-xl bg-slate-800 text-white font-bold hover:bg-slate-700 transition shadow-lg">
                        Tutup Panduan
                    </button>
                </div>
            </div>
        ...
        <!-- Tabel Server (Alpine logic tersendiri) -->
        <div class="glass-effect rounded-2xl p-6 border border-cyan-100 mt-8" x-data="{
            loading: false,
