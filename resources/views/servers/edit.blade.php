                ...
                <form action="{{ route('servers.update', $server->id) }}" method="POST" class="space-y-6 relative z-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-4 top-3 text-slate-400"><i class="fas fa-history"></i></span>
                                <input type="number" name="retention_days" min="1" max="365" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-cyan-200 focus:border-cyan-400 transition-all" 
                                       required value="{{ old('retention_days', $server->retention_days ?? 7) }}">
                            </div>
                            <p class="text-[10px] text-slate-400 mt-1">Lama penyimpanan rekaman di storage node.</p>
                        </div>
                    </div>
